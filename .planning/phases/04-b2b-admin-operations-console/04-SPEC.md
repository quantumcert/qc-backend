...
- canonical tenant profile Asset with public `externalId` equal to the visible uppercase Tenant name, profile metadata, CNPJ/taxId key metadata, legacy `tenant-profile:<tenantId>` fallback and approved event log for blockchain anchoring;
