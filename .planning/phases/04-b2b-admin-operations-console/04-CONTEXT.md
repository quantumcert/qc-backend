...
- perfil do tenant como Asset canonico com `externalId` publico igual ao nome visivel do Tenant em CAIXA ALTA, mantendo fallback para registros legados `tenant-profile:<tenantId>`;
...
- **D-02b:** O `externalId` publico do Asset de perfil do tenant deve ser o nome visivel do Tenant em CAIXA ALTA, pois aparece na Consulta Publica. O backend deve reaproveitar/migrar registros legados `tenant-profile:<tenantId>` sem criar Asset duplicado.
- **D-02c:** `Tenant.name` deve ser normalizado em CAIXA ALTA na criacao/edicao, e o dashboard deve refletir a mesma regra nos campos e exibicoes principais.
